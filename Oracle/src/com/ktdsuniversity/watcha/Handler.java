...
public class Handler {
	...
	private void printRatingsMenus() {
		System.out.println("=".repeat(30));
	...
	private void movieMenuHandler() {
		...
		if (movieMenu == 1) {
			...
			boolean deletedResult = this.moviesService.deleteOneMovieById(movieId);
			if(deletedResult) {
	...
	private void DirectorMenuHandler() {
		...
		if(directorMenu == 1) {
			...
			if(isModified) {
				...
				System.out.println("감독 수정이 실패 하였습니다.");
			}
		}
			...
			String name = this.keyboard.nextLine();
			System.out.println("추가하려는 감독의 프로필 사진 경로를 입력하세요.");
...
//		boolean userIsCreate = usersService.createNewUser("testuser", "테스트 사용자", "", "");
//		if (userIsCreate) {
...
//		boolean ratingIsCreate = ratingsService.createNewRating("testuser", 4.5, "볼만해요", "MV-20240229-0000");
//		if(ratingIsCreate) {
