...
public class SessionFilter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		// 세션을 가져와서 로그인 여부를 판단.
		// Session 을 가져오려면, HttpServletRequest 에서 가져와야한다.
		// request 파라미터는 ServletRequest에서 가져와야 한다.
		// HttpServletRequest is a ServletRequest
		// HttpServletRequest 는 ServletRequest를 상속한 클래서.
		...
		// Servlet(DispatcherServlet)에서 보내준 Response 를 검사하는 필터링 코드 실행.
	}
