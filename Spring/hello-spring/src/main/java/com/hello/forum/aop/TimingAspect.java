...
@Aspect  // AOP 모듈
@Component   // Spring Bean 생성해주는 Annotations
public class TimingAspect {
	 ...
	 *   public : public 접근제어자를 사용하는 것을 대상으로 한다.
	 *   * : 모든 반환타입을 대상으로 한다.
	 *   com.hello.forum..service.*ServiceImpl: com.hello.forum 아래에 있는 모든 패키지 중에서
	 *   										service 패키지 내부에 있는
	 *   										ServiceImpl 로 끝나는 모든 클래스를 대상으로 한다.
...
//	@AfterThrowing
	...
	public Object timingAdvice(ProceedingJoinPoint pjp) throws Throwable {
		...
		try {
			...
			// Controller 로 예외가 전달되고, 마지막으로 ControllerAdvice로 예외가 전달되어
			// 공통 예외를 처리할 수 있기 때문.
			throw e;
		} finally {
			stopWatch.stop();
