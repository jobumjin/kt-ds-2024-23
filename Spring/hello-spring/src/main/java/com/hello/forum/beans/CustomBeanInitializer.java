 ...
 * 개발자가 직접만든 클래스의 Bean 을 Bean Container 에 적재시키는 역할.
 * 
 * @SpringBootConfiguration
 * 	Spring BOOT Application 이 실행될 때 (서버를 Run 시킬 때)
 *  Spring Boot 가 이 애노테이션이 적용된 클래스를 찾아 실행시키고
 *  필요한 bean 들을 Bean Container 에 적재한다.
 */
@SpringBootConfiguration
public class CustomBeanInitializer {
	 ...
	 * 이 메소드가 반환하는 객체를 Bean Container 에 적재를 한다.
	 * 
