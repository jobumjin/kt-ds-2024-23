...
public class HelloBootController {
	...
	public ResponseEntity<String> hello() {
		ResponseEntity<String> responseBody =new ResponseEntity<>("Hello Boot Controller", HttpStatus.OK);
		return responseBody;
	}
...
//		rd.forward(request, response);
		...
		// Spring (Boot) 1개의 Servlet이 내장되어 있다.
		// 내장되어 있는 Servlet이 Controller를 호출
		// 만약, Controller가 변환시킨 데이터가 String 타입이라면
		// Servlet의 코드( RequestDispatcher~~~~; rd.forward(request, response);을 수행한다.
		// 파일의 이름이 반환되었을 경우
