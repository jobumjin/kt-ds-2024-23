...
/**
 * <pre>
 * UserDetailsService 와 PasswordEncoder 를 이용해서
 * 로그인을 요청한 사용자의 인증을 처린한다.
 * 
 ...
 * 
 * AthenticationProvider 인터페이스를 구현해야한다.
 * </pre>
 */
@Component
public class SecurityAuthenticationProvider implements AuthenticationProvider{
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		// UserDetailsService 에서 이메일 정보로 회원의 정보를 조회한다.
		// 만약, 회원의 정보가 존재하지 않다면, UsernameNotFoundException 이 던져진다.
		// UserDetails ==> SecurityUser
		UserDetails userDetails = this.userDetailsService.loadUserByUsername(email);
		
		// 비밀번호 암호화를 위해서 DB에 저장되어있던 salt 를 조회한다.
		// salt 값은 MemberVO 내부에 있고
		// Spring Security 과정 내부에서는 UserDetails에 있고
		// UserDetails 를 상속한 SecurityUser 가 있다.
		// UserDetails 를 SecurityUser로 변환하여 Salt를 얻어온다.
		String salt = ((SecurityUser) userDetails).getMemberVO().getSalt();
		
		// PasswordEncoder 로 로그인에 사용된 비밀번호를 암호화.
		// Spring Security 과정 내부에서 PasswordEncoder 는 SecuritySHA로 정의,.
		...
		// PasswordEncoder 를 이용해서 암호화된 비밀번호와,. 입력한 비밀번호가 같은지 비교
